...
describe('document/utils', () => {
    describe('extractTag', () => {
        it('supports boolean attributes', () => {
            const extracted = utils_1.extractStyleTag('<style test></style>');
            assert.deepStrictEqual(extracted === null || extracted === void 0 ? void 0 : extracted.attributes, { test: 'test' });
        });
        it('supports unquoted attributes', () => {
            const extracted = utils_1.extractStyleTag('<style type=text/css></style>');
            assert.deepStrictEqual(extracted === null || extracted === void 0 ? void 0 : extracted.attributes, {
                type: 'text/css'
            });
        });
        it('does not extract style tag inside comment', () => {
            ...
            `;
            assert.deepStrictEqual(utils_1.extractStyleTag(text), {
        ...
        it('does not extract tags starting with style/script', () => {
            ...
            ></style>
            `;
            assert.deepStrictEqual(utils_1.extractStyleTag(text), null);
        });
        it('is canse sensitive to style/script', () => {
            ...
            `;
            assert.deepStrictEqual(utils_1.extractStyleTag(text), null);
            assert.deepStrictEqual(utils_1.extractScriptTags(text), null);
        });
        it('only extract attribute until tag ends', () => {
            ...
            `;
            const extracted = utils_1.extractScriptTags(text);
            const attributes = (_a = extracted === null || extracted === void 0 ? void 0 : extracted.script) === null || _a === void 0 ? void 0 : _a.attributes;
            assert.deepStrictEqual(attributes, { type: 'typescript' });
        });
        it('can extract with self-closing component before it', () => {
            const extracted = utils_1.extractStyleTag('<SelfClosing /><style></style>');
        ...
        it('can extract with unclosed component after it', () => {
            const extracted = utils_1.extractStyleTag('<style></style><C {#if asd}<p>asd</p>{/if}');
            assert.deepStrictEqual(extracted, {
                start: 7,
                end: 7,
                startPos: {
        ...
        it('extracts top level script tag only', () => {
            ...
            `;
            assert.deepStrictEqual((_a = utils_1.extractScriptTags(text)) === null || _a === void 0 ? void 0 : _a.script, {
        ...
        it('extracts script and module script', () => {
            ...
            <script>b</script>
            `;
            assert.deepStrictEqual(utils_1.extractScriptTags(text), {
                moduleScript: {
                    attributes: {
                        context: 'module'
                    },
    ...
    describe('#getLineAtPosition', () => {
        it('should return line at position (only one line)', () => {
            assert.deepStrictEqual(utils_1.getLineAtPosition(vscode_languageserver_1.Position.create(0, 1), 'ABC'), 'ABC');
        });
        it('should return line at position (multiple lines)', () => {
            assert.deepStrictEqual(utils_1.getLineAtPosition(vscode_languageserver_1.Position.create(1, 1), 'ABC\nDEF\nGHI'), 'DEF\n');
        });
    });
    describe('#updateRelativeImport', () => {
        it('should update path of component with ending', () => {
            const newPath = utils_1.updateRelativeImport('C:/absolute/path/oldPath', 'C:/absolute/newPath', './Component.svelte');
            assert.deepStrictEqual(newPath, '../path/oldPath/Component.svelte');
        });
        it('should update path of file without ending', () => {
            const newPath = utils_1.updateRelativeImport('C:/absolute/path/oldPath', 'C:/absolute/newPath', './someTsFile');
            assert.deepStrictEqual(newPath, '../path/oldPath/someTsFile');
        });
        it('should update path of file going one up', () => {
            const newPath = utils_1.updateRelativeImport('C:/absolute/path/oldPath', 'C:/absolute/path', './someTsFile');
            assert.deepStrictEqual(newPath, './oldPath/someTsFile');
        });
    });
    describe('#getWordAt', () => {
        it('returns word between whitespaces', () => {
            assert.equal(utils_1.getWordAt('qwd asd qwd', 5), 'asd');
        });
        it('returns word between whitespace and end of string', () => {
            assert.equal(utils_1.getWordAt('qwd asd', 5), 'asd');
        });
        it('returns word between start of string and whitespace', () => {
            assert.equal(utils_1.getWordAt('asd qwd', 2), 'asd');
        });
        it('returns word between start of string and end of string', () => {
            assert.equal(utils_1.getWordAt('asd', 2), 'asd');
        });
        it('returns word with custom delimiters', () => {
            assert.equal(utils_1.getWordAt('asd on:asd-qwd="asd" ', 10, { left: /\S+$/, right: /[\s=]/ }), 'on:asd-qwd');
        });
        function testEvent(str, pos, expected) {
            assert.equal(utils_1.getWordAt(str, pos, { left: /\S+$/, right: /[^\w$:]/ }), expected);
        }
        it('returns event #1', () => {
        ...
        it('returns empty string when only whitespace', () => {
            assert.equal(utils_1.getWordAt('a  a', 2), '');
        });
    });
});
