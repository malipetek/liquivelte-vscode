...
describe('CSS Plugin', () => {
    ...
    describe('provides hover info', () => {
        it('for normal css', () => {
            ...
            assert.deepStrictEqual(plugin.doHover(document, vscode_languageserver_1.Position.create(0, 8)), {
                contents: [
                    { language: 'html', value: '<h1>' },
                    '[Selector Specificity](https://developer.mozilla.org/en-US/docs/Web/CSS/Specificity): (0, 0, 1)'
                ],
                range: vscode_languageserver_1.Range.create(0, 7, 0, 9)
            });
        ...
        it('not for SASS', () => {
            ...
            assert.deepStrictEqual(plugin.doHover(document, vscode_languageserver_1.Position.create(0, 20)), null);
        });
        it('not for stylus', () => {
            const { plugin, document } = setup('<style lang="stylus">h1 {}</style>');
            assert.deepStrictEqual(plugin.doHover(document, vscode_languageserver_1.Position.create(0, 22)), null);
        });
        it('for style attribute', () => {
            const { plugin, document } = setup('<div style="height: auto;"></div>');
            assert.deepStrictEqual(plugin.doHover(document, vscode_languageserver_1.Position.create(0, 13)), {
                contents: {
                    kind: 'markdown',
                    value: 'Specifies the height of the content area,' +
                        " padding area or border area \\(depending on 'box\\-sizing'\\)" +
                        ' of certain boxes\\.\n' +
                        '\nSyntax: &lt;viewport\\-length&gt;\\{1,2\\}\n\n' +
                        '[MDN Reference](https://developer.mozilla.org/docs/Web/CSS/height)'
                },
                range: vscode_languageserver_1.Range.create(0, 12, 0, 24)
            });
        });
        it('not for style attribute with interpolation', () => {
            const { plugin, document } = setup('<div style="height: {}"></div>');
            assert.deepStrictEqual(plugin.doHover(document, vscode_languageserver_1.Position.create(0, 13)), null);
        });
    });
    describe('provides completions', () => {
        it('for normal css', () => {
            const { plugin, document } = setup('<style></style>');
            const completions = plugin.getCompletions(document, vscode_languageserver_1.Position.create(0, 7), {
                triggerCharacter: '.'
            });
            assert.ok(Array.isArray(completions && completions.items), 'Expected completion items to be an array');
            ...
            assert.deepStrictEqual(completions.items[0], {
                ...
                textEdit: vscode_languageserver_1.TextEdit.insert(vscode_languageserver_1.Position.create(0, 7), '@charset'),
                tags: []
            });
        });
        it('for :global modifier', () => {
            const { plugin, document } = setup('<style>:g</style>');
            const completions = plugin.getCompletions(document, vscode_languageserver_1.Position.create(0, 9), {
                triggerCharacter: ':'
            });
            const globalCompletion = completions === null || completions === void 0 ? void 0 : completions.items.find((item) => item.label === ':global()');
        ...
        it('for style attribute', () => {
            ...
            const completions = plugin.getCompletions(document, vscode_languageserver_1.Position.create(0, 22), {
                triggerKind: vscode_languageserver_1.CompletionTriggerKind.Invoked
            });
            assert.deepStrictEqual(completions === null || completions === void 0 ? void 0 : completions.items.find((item) => item.label === 'none'), {
        ...
        it('not for style attribute with interpolation', () => {
            const { plugin, document } = setup('<div style="height: {}"></div>');
            assert.deepStrictEqual(plugin.getCompletions(document, vscode_languageserver_1.Position.create(0, 21)), null);
        });
    });
    ...
    describe('provides document colors', () => {
        it('for normal css', () => {
            const { plugin, document } = setup('<style>h1 {color:blue;}</style>');
            const colors = plugin.getColorPresentations(document, {
        ...
        it('not for SASS', () => {
            ...
            assert.deepStrictEqual(plugin.getColorPresentations(document, {
    ...
    it('provides selection range', () => {
        const { plugin, document } = setup('<style>h1 {}</style>');
        const selectionRange = plugin.getSelectionRange(document, vscode_languageserver_1.Position.create(0, 11));
    ...
    it('return null for selection range when not in style', () => {
        ...
        const selectionRange = plugin.getSelectionRange(document, vscode_languageserver_1.Position.create(0, 10));
