...
)
{
    ...
    if (triggerCharacter === '%') {
        // return showCompletionWithRegardsToOpenedTags(
        //     {
        //         liquidIf: createCompletionItems([{ tag: 'if', label: 'if' }]),
        //         liquidUnless: createCompletionItems([{ tag: 'unless', label: 'unless' }]),
        //         liquidFor: createCompletionItems([{ tag: 'for', label: 'for' }]),
        //         liquidEval: createCompletionItems([{ tag: 'liquid', label: 'liquid' }]),
        //     },
        //     svelteDoc,
        //     offset
        // );
        return createCompletionItems([
            { tag: 'if', label: 'if', insertText: 'if $1}\n\t$2\n{/if' },
            { tag: 'unless', label: 'unless', insertText: 'unless $1 %}\n\t$2\n{% endunless' },
            { tag: 'for', label: 'for', insertText: 'for $2 in $1 %}\n\t$3\n{% endfor' },
            { tag: 'liquid', label: 'liquid', insertText: 'liquid $1 %}' }
        ]);
    }
...
function showCompletionWithRegardsToOpenedTags (
    on: {
        ...
        liquidIf?: CompletionList;
        liquidUnless?: CompletionList;
        liquidFor?: CompletionList;
        liquidEval?: CompletionList;
    },
    svelteDoc: SvelteDocument,
    offset: number
)
