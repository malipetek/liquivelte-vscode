...
class DocumentManager {
    ...
    openDocument(textDocument) {
        textDocument = Object.assign(Object.assign({}, textDocument), { uri: utils_1.normalizeUri(textDocument.uri) });
        let document;
    ...
    lockDocument(uri) {
        this.locked.add(utils_1.normalizeUri(uri));
    }
    ...
    updateDocument(textDocument, changes) {
        const document = this.documents.get(utils_1.normalizeUri(textDocument.uri));
        if (!document) {
            throw new Error('Cannot call methods on an unopened document');
        }
