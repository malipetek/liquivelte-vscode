...
export function activate (context: ExtensionContext)
{
    ...
    const runtimeConfig = workspace.getConfiguration('liquivelte.language-server');
    ...
    const clientOptions: LanguageClientOptions = {
        documentSelector: [{ scheme: 'file', language: 'liquivelte' }],
        ...
        initializationOptions: {
            configuration: {
                liquivelte: workspace.getConfiguration('liquivelte'),
    ...
    context.subscriptions.push(
        commands.registerCommand('liquivelte.restartLanguageServer', async () =>
        {
    ...
    async function restartLS (showNotification: boolean)
    {
        ...
        await ls.onReady();
        if (showNotification) {
            window.showInformationMessage('Liquivelte language server restarted.');
        }
        restartingLs = false;
    }
    ...
    languages.setLanguageConfiguration('liquivelte', {
        indentationRules: {
...
function addRenameFileListener (getLS: () => LanguageClient)
{
    workspace.onDidRenameFiles(async (evt) =>
    {
        ...
        if (
            lastPart.includes('.') &&
            !['.ts', '.js', '.json', '.liquivelte'].some((ending) => lastPart.endsWith(ending))
        ) {
            return;
        }
...
function addCompilePreviewCommand (getLS: () => LanguageClient, context: ExtensionContext)
{
    ...
    context.subscriptions.push(
        commands.registerTextEditorCommand('liquivelte.showCompiledCodeToSide', async (editor) =>
        {
            if (editor?.document?.languageId !== 'liquivelte') {
                return;
            }
...
{
    context.subscriptions.push(
        commands.registerTextEditorCommand('liquivelte.extractComponent', async (editor) =>
        {
            if (editor?.document?.languageId !== 'liquivelte') {
                return;
            }
            ...
            window.showInputBox(options).then(async (filePath) =>
            {
                ...
                getLS().sendRequest(ExecuteCommandRequest.type, {
                    command: 'extract_to_svelte_componenta',
...
{
    return new LanguageClient('liquivelte', 'Liquivelte', serverOptions, clientOptions);
}
