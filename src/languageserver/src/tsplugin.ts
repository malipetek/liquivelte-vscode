...
export class TsPlugin {
    ...
    private getEnabledState(): boolean {
        return workspace.getConfiguration('liquivelte').get<boolean>('enable-ts-plugin') ?? false;
    }

    private toggleTsPlugin(enable: boolean) {
        const extension = extensions.getExtension('liquivelte.svelte-vscode');
        if (!extension) {
    ...
    private async askToEnable(enabled: boolean) {
        ...
        if (response === answers[2]) {
            workspace.getConfiguration('liquivelte').update('enable-ts-plugin', true, true);
        } else if (response === answers[1]) {
            workspace.getConfiguration('liquivelte').update('ask-to-enable-ts-plugin', false, true);
        }
    }
}
